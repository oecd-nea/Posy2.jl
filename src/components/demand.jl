...
"""
    makedemand(cname::String, zone::String, n::Node, s::Snapshot; coeff=1.0, shift::Int=0, yearlyconstant::Float64=0., gridlosses=0.,)
...
"""
function makedemand(cname::String, zone::String, n::Node, s::Snapshot; coeff=1.0, shift::Int=0, yearlyconstant::Float64=0., gridlosses=0.)
...
Arguments:
  ...
  * days_threshold: Number of first winter days before summer segment in fixed-profile assembly (used only when `fixed_profile=true`).
...
function makeEV(cname::String, yearly::Number, elec::Node, s::Snapshot; fixed_profile::Bool=true, smart_charging::Bool=false, vehicle_to_grid::Bool=false,
    ...
    @argcheck yearly isa Real "yearly must be Real."
    @argcheck yearly >= 0 "yearly must be >= 0."
    _yearly = Float64(yearly)
    @argcheck gridlosses isa Real "gridlosses must be Real."
    @argcheck 0 <= gridlosses < 1 "gridlosses must be in [0, 1)."
    _gridlosses = Float64(gridlosses)
    @argcheck compensation isa Real "compensation must be Real or left as default."
    ...
    if fixed_profile
        ...
        @argcheck minratio isa Real "minratio must be Real."
