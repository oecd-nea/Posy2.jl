...
function makesnapshot(; optimize::Bool=false)
    ...
    makedispatchable("CCGT", "CCGT", elec, co2, snap; cap=100.0, construction_profile=1.0)
    makeintermittentsource("Onwind gen", "Onwind", elec, co2, snap; cap=100.0, weatheryear=2019, construction_profile=1.0)
    makebatteries(
    ...
    !optimize && return snap, elec, co2
    Nosy.optimize!(snap, cost(snap))
    return extract(snap), elec, co2
end
...
makeintermittentsource("Plant A", "Onwind", elec, co2, s; cap=50.0, weatheryear=2019, construction_profile=1.0)
