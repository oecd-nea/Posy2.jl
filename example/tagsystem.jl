...
# Do builders attach the function tags PP relies on 
@assert Nosy.hastag(wind, :function, "generation") && Nosy.hastag(wind, :function, "intermittent")
@assert Nosy.hastag(bat, :function, "storage") && Nosy.hastag(bat, :function, "generation")
@assert Nosy.hastag(ccgt, :function, "generation") && Nosy.hastag(ccgt, :function, "dispatchable")

# Can we find components snapshot-wide with a single function tag
intermittent = Nosy.getcomponents(s; with=[:function => "intermittent"])
storage = Nosy.getcomponents(s; with=[:function => "storage"])
dispatchable = Nosy.getcomponents(s; with=[:function => "dispatchable"])
...
all_generation = Nosy.getcomponents(s; with=[:function => "generation"])
generation_without_storage = Nosy.getcomponents(s; with=[:function => "generation"], without=[:function => "storage"])
...
zone1_intermittent = Nosy.getcomponents(s, "ZONE1"; with=[:function => "generation", :function => "intermittent"])
@assert haskey(zone1_intermittent, "Onwind gen ZONE1")

zone1_generation = Nosy.getcomponents(s, "ZONE1"; with=[:function => "generation"])
...
intermittent_all = Nosy.getcomponents(s; with=[:function => "intermittent"])
@assert length(intermittent_all) == 2
@assert haskey(intermittent_all, "Plant A ZONE1")

# Does makepriceinterco tag function and neighbor zone
price_ic = makepriceinterco("ZONE2", elec, 100.0, 100.0, s)

@assert Nosy.hastag(price_ic, :function, "interconnection")
@assert Nosy.hastag(price_ic, :function, "priceinterconnection")
@assert Nosy.hastag(price_ic, :function, "foreign")
@assert Nosy.hastag(price_ic, :neighbor, "ZONE2")

# Can we find a price IC by external zone tag, and by local node + zone tag
zone2_price_ics = Nosy.getcomponents(s; with=[:function => "interconnection", :function => "priceinterconnection", :neighbor => "ZONE2"])
@assert length(zone2_price_ics) == 1
@assert haskey(zone2_price_ics, Nosy.name(price_ic))

zone1_price_ics = Nosy.getcomponents(s, "ZONE1"; with=[:function => "interconnection", :function => "priceinterconnection", :neighbor => "ZONE2"])
