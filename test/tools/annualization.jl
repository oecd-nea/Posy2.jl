...
@testset "Annualization" begin
    ...
    let
        ...
        # eac = overnight_cost * construction_factor * corrected_crf.
        ...
        # decom_cost = overnight_cost*deco_ratio*(1+r)^(-n)*decommissioning_factor*corrected_crf; profile 1.0 gives factor 1.
    ...
    let
        ...
        @test isapprox(Posy2.construction_factor(0.05, fill(1 / 3, 3)), 1.103375; rtol=1e-10)
        ...
        @test_throws ArgumentError Posy2.construction_factor(0.05, Float64[])
        @test_throws ArgumentError Posy2.construction_factor(0.05, [Inf, -Inf])

        # Semicolon strings are workbook syntax; they are parsed when the workbook is read, not here.
        @test_throws ArgumentError Posy2.construction_factor(0.05, "0.3;0.4;0.3")
        @test_throws ArgumentError Posy2.decommissioning_factor(0.05, "0.5;0.5")
    end

    # Normalization must not mutate the caller's vector.
    let
        shares = [0.333, 0.333, 0.333]
        Posy2.construction_factor(0.05, shares)
        @test shares == [0.333, 0.333, 0.333]
    end
