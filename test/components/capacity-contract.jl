...
@testset "Capacity contract" begin
    ...
    let
        ...
        makedispatchable("CCGT", elec, s; co2_node=co2, tech_column="CCGT", cap=nothing, maxcap=40.0, om_var_cost=1.0)
        makeintermittentsource("PV", elec, s; co2_node=co2, tech_column="PV", cap=nothing, maxcap=30.0, profile=1.0)
        ...
        gas = makedispatchable("CCGT", telec, t; co2_node=tco2, tech_column="CCGT", cap=ini, om_var_cost=1.0)
        pv = makeintermittentsource("PV", telec, t; co2_node=tco2, tech_column="PV", cap=ini, profile=1.0)
    ...
    function extracted_without_components()
        ...
        makedispatchable("Other", eelec, e; co2_node=eco2, tech_column="CCGT", cap=100.0, om_var_cost=1.0)
    ...
    let
        ...
        @test_throws "`cap` was given a snapshot with no component named \"CCGT E1\" to inherit port \"output\" from" makedispatchable("CCGT", elec, s; co2_node=co2, tech_column="CCGT", cap=empty_ini)
        ...
        @test_throws "no component named \"EPR E1\" to inherit port \"output\"" makenuclear(
            "EPR", elec, s; co2_node=co2, tech_column="Nuclear", cap=empty_ini)
        @test_throws "no component named \"PV E1\" to inherit port \"output\"" makeintermittentsource(
            "PV", elec, s; co2_node=co2, tech_column="PV", cap=empty_ini, profile=1.0)
    ...
    let
        ...
        makedispatchable("CCGT", elec, s; co2_node=co2, tech_column="CCGT",
        ...
        @test_throws "`cap` was given a snapshot that is not extracted" makedispatchable(
            "CCGT", telec, t; co2_node=tco2, tech_column="CCGT", cap=s)
        ...
        @test_throws "`uc` was given a snapshot that is not extracted" makedispatchable(
            "CCGT", telec, t; co2_node=tco2, tech_column="CCGT", cap=100.0, uc=s, unit_size=50.0)
    end

    # A same-named source component of another kind lacks the requested port.
    let
        s, elec, _, co2 = contract_snapshot()
        makedispatchable("Twin", elec, s; co2_node=co2, tech_column="CCGT", cap=100.0)
        Nosy.optimize!(s, cost(s))
        ini = extract(s)
    ...
    let
        ...
        t, telec, _, tco2 = contract_snapshot()
        replayed = makedispatchable("CCGT", telec, t; co2_node=tco2, tech_column="CCGT",
        ...
        fresh = makedispatchable("CCGT", uelec, u; co2_node=uco2, tech_column="CCGT",
        ...
        @test_throws "requires a fixed capacity" makedispatchable("CCGT", velec, v; co2_node=vco2, tech_column="CCGT",
        ...
        @test_throws "`uc` was given a snapshot with no component named \"CCGT E1\"" makedispatchable(
            "CCGT", welec, w; co2_node=wco2, tech_column="CCGT", cap=10.0, uc=noc, unit_size=50.0)
        ...
        makedispatchable("CCGT", yelec, y; co2_node=yco2, tech_column="CCGT", cap=100.0, om_var_cost=1.0)
        ...
        @test_throws "carries no unit commitment behavior on port \"output\"" makedispatchable(
            "CCGT", xelec, x; co2_node=xco2, tech_column="CCGT", cap=10.0, uc=nouc, unit_size=50.0)
    end
    ...
    let
        s, elec, _, co2 = contract_snapshot()
        makenuclear("EPR", elec, s; co2_node=co2, tech_column="Nuclear",
            cap=100.0, uc=true, unit_size=50.0, min_power=0.5, om_var_cost=1.0)
        makedemand("Load", "Z1", elec, s; profile_multiplier=0.0, annual_flat_demand=8760.0 * 60.0)
        Nosy.optimize!(s, cost(s))
        ini = extract(s)
        ...
        t, telec, _, tco2 = contract_snapshot()
        replayed = @test_logs (:warn,) match_mode=:any makenuclear("EPR", telec, t; co2_node=tco2, tech_column="Nuclear",
            cap=ini, uc=ini, unit_size=50.0, om_var_cost=1.0,
    ...
    let
        s, elec, h2, co2 = contract_snapshot()
        gas = makedispatchable("CCGT", elec, s; co2_node=co2, tech_column="CCGT", cap=0.0)
    ...
    let
        s, elec, _, co2 = contract_snapshot()
        @test !isnothing(makedispatchable("In range", elec, s; co2_node=co2, tech_column="CCGT",
            cap=50.0, mincap=10.0, maxcap=100.0))
        @test_throws ArgumentError makedispatchable("Below", elec, s; co2_node=co2, tech_column="CCGT",
            cap=5.0, mincap=10.0)
        @test_throws ArgumentError makedispatchable("Above", elec, s; co2_node=co2, tech_column="CCGT",
